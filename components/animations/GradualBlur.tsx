...
const getResponsiveDimensionValue = (
  ...
  const mobileKey = `mobile${cap}` as ResponsiveConfigKey;
  const tabletKey = `tablet${cap}` as ResponsiveConfigKey;
  const desktopKey = `desktop${cap}` as ResponsiveConfigKey;

  if (width <= 480 && config[mobileKey]) {
    return config[mobileKey] as string;
  }
  if (width <= 768 && config[tabletKey]) {
    return config[tabletKey] as string;
  }
  if (width <= 1024 && config[desktopKey]) {
    return config[desktopKey] as string;
  }
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      setVal(getResponsiveDimensionValue(config, key, window.innerWidth));
    };
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      transition: config.animated
        ...
        : undefined,
      zIndex: isPageTarget ? config.zIndex + 1 : config.zIndex,
      ...config.style,
    };
